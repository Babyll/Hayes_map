(function (window, undefined) {
    var MapsLib = function (options) {
        ...
        var minDate = moment("Jan 1 1900"); // Jan 1st 2010
        var maxDate = moment("Jan 1 1950"); //now

        //starting values
        var startDate = moment("Jan 1 1900"); //past 3 months
        var endDate = moment("Jan 1 1950"); //now
    ...
    MapsLib.prototype.submitSearch = function (whereClause, map) {
        ...
        self.searchrecords = new google.maps.FusionTablesLayer({
            ...
            styleId: 2,
            templateId: 5 //setting this to the number in the url next to 'tmplt=' given by "Publish" on fusion tables, lets you use html from fusion tables
        });
    ...
    MapsLib.prototype.doSearch = function () {
        ...
        //-----custom filters-----
        self.whereClause += " AND 'Date of Death' >= '" + $('#startDate').html() + "'";
        self.whereClause += " AND 'Date of Death' <= '" + $('#endDate').html() + "'";
        var fName_search = $("#fName_search").val().replace("'", "\\'");
        var lName_search = $("#lName_search").val().replace("'", "\\'");
        var CoD_search = $("#CoD_search").val().replace("'", "\\'");
        var physician_search = $("#physician_search").val().replace("'", "\\'");
        if (fName_search != '')
            self.whereClause += " AND 'Name First' contains ignoring case '" + fName_search + "'";
        if (lName_search != '')
            self.whereClause += " AND 'Name Last' contains ignoring case '" + lName_search + "'";
        if (CoD_search != '')
            self.whereClause += " AND 'Cause of Death' contains ignoring case '" + CoD_search + "'";
        if (physician_search != '')
            self.whereClause += " AND 'Certifying Physician' contains ignoring case '" + physician_search + "'";
